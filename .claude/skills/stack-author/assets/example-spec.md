...
- Group imports stdlib → third-party → first-party with one blank line between groups.
- Sort alphabetically within each group.
- Run `isort` with the `black` profile in pre-commit so the order is enforced, not negotiated.

## Don't

- Use wildcard imports (`from x import *`) outside `__init__.py` re-exports.
- Reorder imports by hand — let the tool own it.
- Mix `import x` and `from x import y` styles within a group.
